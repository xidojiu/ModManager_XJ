    ...
    public class ModManager_XJMod : UserMod2
    {
        ...
        private static void RegisterStrings()
        {
            try
            {
                ...
                {
                    // IL2CPP 下 Location 可能为空，忽略
                }

                // 如果 .po 文件存在，从它加载
                if (poPath != null && File.Exists(poPath))
                {
                ...
                // fallback：注册英文
                RegisterEnglishFallback();
            }
            catch (System.Exception e)
            {
                Debug.LogError("[MM] 注册本地化字符串失败: " + e);
                // 极端 fallback
                try { RegisterEnglishFallback(); }
                catch { }
            }
        }

        // 解析 .po 文件并用 Strings.Add() 注册
        private static int LoadPoFile(string path)
        {
            ...
            for (int i = 0; i < lines.Length; i++)
            {
                ...
                {
                    // 保存上一个条目
                    if (currentKey != null && currentTranslation != null)
                    {
                        try
                        {
                            Strings.Add(currentKey, new LocString(currentTranslation));
                            count++;
                        }
                        catch
                        {
                    ...
                    int start = "msgctxt \"".Length;
                    int end = line.LastIndexOf('"');
                    if (end > start)
                    {
                        currentKey = line.Substring(start, end - start);
                        currentTranslation = null;
                    }
                }
                else if (line.StartsWith("msgstr \""))
                {
                    int start = "msgstr \"".Length;
                    int end = line.LastIndexOf('"');
                    if (end > start)
                    {
                        currentTranslation = line.Substring(start, end - start);
                    }
                }
            }
        ...
        // 英文 fallback
        private static void RegisterEnglishFallback()
        {
            ...
            Debug.Log("[MM] 已注册英文 fallback 本地化字符串");
        }
    }
}
